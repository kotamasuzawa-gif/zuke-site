...
export default function News() {
  return (
    <section id="news" className="px-4 md:px-6 py-16 max-w-6xl mx-auto" aria-label="お知らせ">
      <h2 className="font-[family-name:var(--font-geist-mono)] text-2xl md:text-3xl tracking-[0.12em] text-neutral-100 mb-8">
        NEWS
      </h2>
      <ul className="border-t border-[#2a2a28]">
        {news.map((n) => (
          <li
            key={n.date + n.title}
            className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-8 py-5 border-b border-[#2a2a28]"
          >
